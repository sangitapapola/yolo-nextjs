...
const TensorFlowComponent = () => {
  ...
  const imageToTensor = async (imageElement) => {
    ...
    const scale = Math.min(TARGET_WIDTH / imgWidth, TARGET_HEIGHT / imgHeight);
  ...
  const drawBoundingBoxes = async (imageElement, detections) => {
    ...
    let displayWidth, displayHeight, originalWidth, originalHeight;
    
    if (imageElement.videoWidth) {
      // Video element
      displayWidth = imageElement.videoWidth;
      displayHeight = imageElement.videoHeight;
      originalWidth = imageElement.videoWidth;
      originalHeight = imageElement.videoHeight;
    } else {
      // Image element
      displayWidth = imageElement.naturalWidth || imageElement.width;
      displayHeight = imageElement.naturalHeight || imageElement.height;
    ...
    // Calculate how the original image was scaled and padded when converted to tensor
    const scale = Math.min(TARGET_WIDTH / originalWidth, TARGET_HEIGHT / originalHeight);
    const scaledWidth = originalWidth * scale;
    const scaledHeight = originalHeight * scale;
    const padX = (TARGET_WIDTH - scaledWidth) / 2;
    const padY = (TARGET_HEIGHT - scaledHeight) / 2;

    detections.forEach(({ box, label }) => {
      let [x, y, width, height] = box;
      
      // Remove the padding offset and scale back to original size
      x = (x - padX) / scale;
      y = (y - padY) / scale;
      width = width / scale;
      height = height / scale;

      // Skip boxes that are outside the original image bounds (due to padding)
